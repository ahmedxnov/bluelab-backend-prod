"""queue schema — Procrastinate 3.9.0, pinned

Revision ID: 0002_queue_schema
Revises: 0001_initial_schema
Created: 2026-07-30

data/04 §4: Procrastinate owns its tables, and its library migrations are
**pinned to the library version and wrapped as steps of our lineage** — one
lineage migrates the whole database, so a restore plus `alembic upgrade head`
always converges. A queue schema applied out-of-band would mean a restored
database that looks complete and cannot enqueue.

This is the banked Definition-of-Done item from quality/08 §4 and data **F-9**
("pin the Procrastinate `defer` API against the pinned library version")
discharged for the schema half: the SQL below is verbatim
`SchemaManager.get_schema()` from Procrastinate **3.9.0**, captured rather
than referenced, so upgrading the library cannot silently change what a restore
produces. The *statement* half lives in `platform/queue/enqueue.py`.

**Why our own SQL rather than `procrastinate schema --apply`:** the CLI opens its
own connection and commits independently. That is fine for a one-off, and wrong
for a lineage — a restore runbook that has to remember a second command is a
runbook that eventually forgets it.

The queue tables carry no customer scope and are excluded from the RLS
completeness check (`tools/generate_rls_policies.py`). They are library-owned;
`docs/open-items.md` OI-17 tracks that they run without policies.
...
revision: str = "0002_queue_schema"
down_revision: str | None = "0001_initial_schema"
branch_labels: str | None = None
depends_on: str | None = None

PROCRASTINATE_VERSION = "3.9.0"
"""Pinned. If this disagrees with the installed library, the queue schema and the
enqueue statement have drifted apart — see data F-9."""
