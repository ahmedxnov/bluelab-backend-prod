...
The drill shall persist separate scenario and rubric generation states, each with its request identity and status (`none`, `running`, `succeeded`, or `failed`), while permitting at most one running stage. It shall persist a safe failure reason only while one stage is failed. When scenario or rubric generation fails, the system shall show the author the failure with a retry, shall substitute no fallback content, and shall not allow a drill to publish without successfully generated scenario and rubric ([interview policy]).
